...
- [Linux productivity tools](lisa19_maheshwari.pdf)
...
- script to generate wallpaper and new tab content, like interesting quotes, puzzles, tweets etc.
...
- The pokemon background thing but with kitty because kitty now supports backgroud. make it in go/rust also allow plugins so that its not limited to pockemon but ppl can use anything any set of images etc.
